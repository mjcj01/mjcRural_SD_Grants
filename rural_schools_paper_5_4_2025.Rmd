---
title: "Federal Funds and Rural School Districts"
...
funding disparities for school districts serving low-income, nonwhite, and rural students. Rural school
districts are tasked with overcoming systemic deficiencies in infrastructure, access to government
services, and high poverty rates. This literature review will analyze three main areas of rural education
research and policy: redefining rurality, demography and the role of community in rural school districts,
and the equity of federal education policies affecting rural school districts.

## Defining Rurality

There is a lack of attention paid to rural education from policymakers and scholars. Ignoring this population has resulted in inequal and inequitable material conditions for rural school districts and their communities. Kai Schafft, a rural sociologist of education (2016), argues that rural Americans, their schools, and public school students exist on the periphery of scholars’ and policymakers’ attention, despite representing a significant proportion of the population. The author claims that education policy programs, like the Rural Low Income School (RLIS) program and Small, Rural School Achievement (SRSA) programs exist as the result of rural school districts failing to meet policy goals and achievement guidelines established for all public schools, rather than a coherent policy vision specifically for rural schools. Moreover, Schafft points to the disparity in scholarly research and education specialization programs at top universities, noting that articles and programs are focused more on urban areas than on rural ones (2016). 
...
The Rural Education Achievement Program (REAP) began under NCLB and continued with 
ESSA. REAP provides states with federal funded grants which are then distributed to rural LEA’s. The Rural Education Achievement Program used “Urban-Centric” locale codes based on students’ geographic proximity to urbanized areas under ESSA. This reductionist approach to defining “rural” does not properly encompass characteristic differences that persist between urban and rural school districts. Existing research supports conceptualizing rurality based on environmental, social, and economic resources available to a community (Nelson and Ngyuen 2023). Hoggart (1990) argues to use contrasting differences such as rich and poor local governments, employment sectors being monopolistic or competitive, and whether the settlement is experiencing social disintegration or vitality. Policymakers and scholars need to encompass more than just geographic distance to an urban center when aiming to define “rural”.  

The last federal education funding policy to be discussed in this literature is Obama’s Race to the Top (RTT). RTT was a $4.35 billion grant program that encouraged states to enter a “voluntary” competition for federal funding. As a result of the underfunding of education across the U.S., one can ask the question if this grant program was truly “voluntary” (Kolbe and Rice 2012). States applied for these grants by submitting reform plans to be scored quantifiably based on rubric by the U.S. Department of Education. States received points for prioritizing STEM, investing in “great teachers and leaders”, and improving standards and assessments, along with several other accountability factors. Race to the Top – District was a smaller and more localized federally funded grant program. This program provided grant funding to LEA’s that proposed personalized plans based on their districts need’s. These proposals were encouraged to promote teamwork, collaboration, and critical thinking in the classroom. Both grant programs suffered the same fate for rural school districts since the programs relied on a proposal system. Competitive grant programs favor urban and suburban school districts that have greater resources for writing competitive grant applications than rural school districts (Brenner 2016).  
...
nces_locale_classifications %>%
  ...
  tab_footnote("Table 1: Data from NCES Locale Classifications.")
```
...
Lastly, IDEA enrollment data is provided from the 2021-2022 Civil Rights Data Collection (CRDC) survey, which is administered through the U.S. Department of Education's Office for Civil Rights. This is a universal survey--that is, all public schools must respond--done at the school level that is conducted every other school year. It collects data used to monitor civil rights enforcement, including data related to discipline, gifted education, course offerings, and special education.

## Methods

Funding categories from the NCES School District Finance Survey will be picked and coded as funding one of four categories:
...
```{r tot_stu_demog, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Student Enrollment Percentages Across Pennsylvania by Locale. Data from NCES Common Core of Data, 2021-22 School Year."}
library(gt)
...
pa_enrollment %>%
  ...
  tab_footnote(footnote = "Table 2: Data from NCES Common Core of Data, 2021-22 School Year")
```

This is a trend that generally matches the communities these school districts are situated in (see Figure 1). Rural school districts also tend to have predominantly White populations reside in them and low percentages of other ethnoracial groups. There is also far less variation in how diverse these places are; while suburban and urban school districts have a diverse district-wide enrollment (though perhaps school-level enrollment within the district is not diverse), no rural school district has a White student enrollment percentage under 50%.
...
Additionally, this is also a broad spatial pattern for rural school districts across the state. Only two districts have a non-White ethnoracial group make up more than 25% of their student population; 29% of students enrolled in both Pocono Mountain and Upper Adams School Districts are Hispanic.
...
poverty_data %>%
  ...
  tab_footnote(footnote = "Table 3: Data from the U.S. Census Bureau's Small Area Income and Poverty Estimates (SAIPE).")
```
...
Considering the economic characteristics of rural school districts (and all school districts, really) is critical, as students living in poverty require more resources to lift them to their potential (Ladd, 2012). School meal programs, after-school programming, and academic support programs all require additional funds to operate at their full potential. With this in mind, it would be expected that school districts with a higher poverty rate among its students would receive additional funding in certain funding categories than districts with a lower rate, either because of an explicit piece of the funding allocation formula that accounts for poverty (such as Title I funding) or because school districts with a higher poverty rate have higher demand for certain programs (such as school meal programs).
...
```{r idea_enr_locale_plot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Data from the U.S. Department of Education's Civil Rights Data Collection (CRDC), 2021-22 collection year."}
crdc_spled_enr %>%
  ...
  merge(.,
        pa_directory %>%
          ...
          select(-urban_centric_locale),
        by.x = "LEAID", by.y = "leaid") %>%
  mutate("enr_pct" = IDEA_enr / enrollment) %>%
  filter(race != "Total") %>%
  ggplot(aes(x = enr_pct, fill = locale)) +
  geom_density(alpha = 0.25) +
  facet_wrap(vars(race)) +
  scale_x_continuous(limits = c(0, 1),
                     labels = scales::percent) +
  scale_fill_discrete(name = "Locale") +
  labs(x = "IDEA Enrollment %") +
  theme_minimal() +
  theme(axis.text.y = element_blank(),
        axis.title.y = element_blank(),
        text = element_text(family = "Tinos"),
  ...
  #         IDEA_val = sum(IDEA_enr) / sum(enrollment),
  #         IDEA_pct = paste(round(sum(IDEA_enr) / sum(enrollment) * 100, digits = 2), "%", sep = "")) %>%
  # select(race, locale, IDEA_val) %>%
  # rename(Race = "race",
  #        Locale = "locale",
  #        `% Enrolled in IDEA` = "IDEA_val") %>%
...
Rural school districts' IDEA enrollment patterns are not dissimilar from that of other locales. They are, nevertheless, troubling; Black and Hispanic students are enrolled IDEA programs in rural and urban school districts at disproportionately high rates. Despite making up 2.92% of enrolled students in rural school districts specifically, Black students account for 22.19% of IDEA enrollment in those same schools; that is a staggering 7.6 times difference. Hispanic students make up 5.61% of enrolled students in rural school districts, and yet make up 20.73% of students enrolled in IDEA in those districts. Mis-classification of Black and Hispanic students into special education programs has long been a practice that internally segregates schools (i.e., the school-wide demographics are racially integrated, but within the school programming is still segregated) (Parker, 2024), as students in special education programs often end up in separate classrooms for special education-related classwork.

# Results

As a starting point for analysis, linear regression was tested to see the correlation between state funding in the five selected categories and federal funding in rural school districts. If federal funding were behaving as an equalizer, the expectation is there would be a negative relationship between federal and state funding in each category; as you can increase state funding in a category, federal funding would decrease. The equation for this model is

$$
{State \ Funding}_i \ \sim \ {Federal \ Funding}_i
$$
where i represents the funding category.
...
initial_reg_table %>%
  ...
  tab_header("State Funding ~ Federal Funding Regression Results") %>%
  tab_footnote("Table 4.")
```
...
fed_sta_comp %>%
  ...
  rename(Locale = "locale",
         `Combined Rev. / Enrolled` = "tot_per_enr",
         `Federal Rev. / Enrolled` = "fed_per_enr",
         `State Rev. / Enrolled` = "sta_per_enr") %>%
  gt() %>%
  cols_align(align = "center") %>%
  tab_header("State and Federal Revenues, by Locale") %>%
  tab_footnote("Table 5. Data from the NCES School District Finance Survey. 'Combined' here refers to federal + state revenue, not all revenue for a school district.")
```
...
To analyze racial composition and nutrition funding, a model was assembled that correlated federal meal revenue with enrollment percentages for four ethnoracial groups: Asian, Black, Hispanic, and White students. The equation for this model is

$$
{Federal \ Funding} \ \sim \ E_{\ Asian} \ + \ E_{\ Black} \ + \ E_{\ Hispanic} \ + \ E_{\ White}
$$
where E is enrollment of a specified ethnoracial group.
...
nutrition_race_reg %>%
  ...
  rename(`p value` = "Pr(>|t|)") %>%
  gt() %>%
  tab_row_group(label = "Rural", rows = Locale == "Rural") %>%
  tab_row_group(label = "Town", rows = Locale == "Town") %>%
  tab_row_group(label = "Suburban", rows = Locale == "Suburban") %>%
  tab_row_group(label = "Urban", rows = Locale == "Urban") %>%
  cols_hide("Locale") %>%
  tab_header("Nutrition Funding ~ Enrollment by Race, by Locale") %>%
  tab_footnote("Table 6. Data from NCES School District Finance Survey and NCES Common Core of Data.")
```
...
nutrition_pov_rev %>%
  mutate(" " = "Student Poverty Rate") %>%
  tibble::rownames_to_column(., "test") %>%
  filter(!grepl("Intercept", test)) %>%
  select(Locale, Estimate, `Std. Error`, ` `, `Pr(>|t|)`) %>%
  relocate(` `) %>%
  rename(`p value` = "Pr(>|t|)") %>%
  gt() %>%
  tab_row_group(label = "Rural", rows = Locale == "Rural") %>%
  ...
  tab_row_group(label = "Urban", rows = Locale == "Urban") %>%
  cols_hide("Locale") %>%
  tab_header("Nutrition Funding ~ Student Poverty Rate, by Locale") %>%
  tab_footnote("Table 7. Data from NCES School District Finance Survey and U.S. Census Bureau Small Area Income and Poverty Estimates.")
```
...
As mentioned previously, Black and Hispanic students are disproportionately represented in special education programs. IDEA enrollment in rural Pennsylvania school districts follow this pattern.
...
for (i in c("Rural", "Town", "Suburban", "Urban")) {
  ...
  idea_race_reg <- df$coefficients[2:5,] %>%
  as.data.frame() %>%
  select(-`t value`) %>%
  rename(`p value` = "Pr(>|t|)") %>%
...
idea_race_reg %>%
  ...
  tab_row_group(label = "Urban", rows = Locale == "Urban") %>%
  cols_hide("Locale") %>%
  tab_header("IDEA Funding ~ IDEA Enrollment by Race, by Locale") %>%
  tab_footnote("Table 8. Data from NCES School District Finance Survey and U.S. Department of Education Civil Rights Data Collection (CRDC).")
```
...
for (i in c("Rural", "Town", "Suburban", "Urban")) {
  ...
  mutate(" " = "Student Poverty Rate",
         "Locale" = i)
  idea_pov_reg <- rbind(df, idea_pov_reg)
}
...
idea_pov_reg %>%
  ...
  tab_row_group(label = "Urban", rows = Locale == "Urban") %>%
  cols_hide("Locale") %>%
  tab_header("IDEA Funding ~ Student Poverty Rate, by Locale") %>%
  tab_footnote("Table 9. Data from NCES School District Finance Survey, the U.S. Department of Education Civil Rights Data Collection (CRDC), and the U.S., and the U.S. Census Bureau Small Area Income and Poverty Estimates (SAIPE).")
```

```{r idea_pov_plot, echo = FALSE, message = FALSE, warning = FALSE, fig.cap=""}
crdc_spled_enr %>%
  pivot_longer(cols = c(Hispanic, White, Black, Asian)) %>% 
  merge(., poverty_data, by = "LEAID") %>%
  merge(., fed_rev %>% rename(cat = "name"), by.x = "LEAID", by.y = "LEAID") %>%
...
A child's ability to learn in an inclusive and safe environment should be an "accident of geography"; that is, students should have access to a quality education they rightfully deserve regardless of where they are born and where they are living while going to school. Some funding categories have a more immediate tie to civil rights, such as IDEA funding. Students in need of learning accommodations have a right to a "free appropriate public education" under federal law. All federal funding ties to civil rights through the federal government's enforcement of civil rights protections for students and staff; disregard or unwillingness to comply with civil rights protections eventually leads to loss of federal funding. The lack of federal education policy specifically aimed at addressing the needs of rural school districts fails to promote equal and equitable opportunities for rural students, especially from low-income or minority backgrounds. Continued disinvestment in rural areas reflects a system that neglects marginalized communities. Federal education policy that relies on reductionist definitions of “rural” do not consider needs distinct to rural communities and ignores racial and economic disparities.  
